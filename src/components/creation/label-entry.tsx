...
export default function UpsertLabelEntry({ uuid, title, colorHex, onSuccess }: { uuid?: string, title?: string, colorHex?: string, onSuccess: () => void }) {
    ...
    const errMsg = loadLabelErr ? `# Something wrong
...
` : undefined;
    ...
    return (errMsg ? <InitError errMarkdown={errMsg} /> :
        ...
        >
            <Form.Description text="Give title and color hex for this label.
Leaving color hex blank will generate random one color." />
            <Form.TextField
                ...
                onBlur={(event) => {
                    if (event.target.value?.length === 0) {
                        ...
                        allLabels !== undefined && allLabels.filter(
                            label => label.title == event.target.value
                                && label.uuid !== uuid
                        ).length > 0
                    ) {
            ...
            <Form.TagPicker id="preview" title="Label Preview" value={previewTitle ? [previewTitle] : []}>
                {previewColorHex && previewTitle &&
                    <Form.TagPicker.Item
                        key={previewTitle} title={previewTitle} value={previewTitle}
